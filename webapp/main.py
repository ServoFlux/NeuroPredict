...
PRETTY_LABELS = {'no_wmd': 'No White Matter Disease', 'early_wmd': 'Early White Matter Disease', **ETIOLOGY_LABELS}
...
def _clinical_groups_for_template() -> list[dict[str, object]]:
    ...
    for category in CATEGORY_ORDER:
        fields = [{'name': f.name, 'label': f.label, 'kind': f.kind, 'help': f.help} for f in CLINICAL_FIELDS if f.category == category]
        if fields:
...
def health() -> dict[str, object]:
    return {'status': 'ok', 'model_loaded': predictor is not None, 'model_path': str(DEFAULT_MULTIMODAL_MODEL_PATH)}
...
def _run_prediction(saved_path: Path, answers: dict[str, float], filename: str, token: str) -> dict[str, object]:
    ...
    if predictor is None:
        ctx['error'] = 'No trained model is available. Run `python scripts/train_demo.py` to train the demo models, then restart the server.'
        return ctx
    ...
    exp = predictor.explain_path(saved_path, answers, prediction, overlay_png, preview_png)
    ctx['preview_url'] = f'/static/previews/{preview_png.name}'
    ctx['overlay_url'] = f'/static/previews/{overlay_png.name}'
    ctx['explanation'] = {'original_shape': '×'.join((str(d) for d in exp.original_shape)), 'processed_shape': '×'.join((str(d) for d in exp.processed_shape)), 'slice_index': exp.slice_index, 'attention_pct': round(exp.attention_fraction * 100, 1), 'clip_percentiles': predictor.preprocess.clip_percentiles}
    ctx['attribution'] = {'combined': round(attr.combined * 100, 1), 'baseline': round(attr.baseline * 100, 1), 'image_delta': round(attr.image_delta * 100, 1), 'clinical_delta': round(attr.clinical_delta * 100, 1), 'image_share': round(attr.image_share * 100), 'clinical_share': round(attr.clinical_share * 100)}
    ...
    cause_probs = [{'label': _pretty(name), 'pct': round(p * 100, 1)} for name, p in sorted(prediction.probabilities.items(), key=lambda kv: kv[1], reverse=True) if name != 'no_wmd']
    ctx['result'] = {'label': prediction.label, 'label_pretty': _pretty(prediction.label), 'confidence': round(prediction.confidence * 100, 1), 'wmd_probability': round(attr.combined * 100, 1), 'probabilities': {_pretty(name): round(p * 100, 1) for name, p in prediction.probabilities.items()}, 'cause_probs': cause_probs, 'is_positive': is_positive, 'severity': {'level': severity.level, 'description': severity.description} if severity is not None else None, 'next_steps': ETIOLOGY_NEXT_STEPS.get(prediction.label, ETIOLOGY_NEXT_STEPS['no_wmd'])}
    _latest_digitized.update(label_pretty=ctx['result']['label_pretty'], wmd_pct=ctx['result']['wmd_probability'])
    return ctx
def _film_to_volume_path(image_path: Path, token: str, cols: int, depth: int) -> Path:
...
async def predict(request: Request) -> HTMLResponse:
    ...
    if scan is None or not filename or (not _has_suffix(filename, ALLOWED_SUFFIXES)):
        ctx = _empty_context(filename)
        ctx['error'] = 'Unsupported file type. Please upload a NIfTI (.nii/.nii.gz) or DICOM (.dcm) scan.'
...
def digitizer(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(request, 'digitizer.html', {'disclaimer': RESEARCH_DISCLAIMER, 'model_loaded': predictor is not None, 'clinical_groups': _clinical_groups_for_template(), 'latest': _latest_digitized})
...
async def digitizer_submit(request: Request) -> HTMLResponse:
    ...
    if sheet is None or not filename or (not _has_suffix(filename, FILM_SUFFIXES)):
        ctx = _empty_context(filename)
        ctx['error'] = 'Please upload a photo of the film sheet (PNG/JPEG).'
        return templates.TemplateResponse(request, 'result.html', ctx)
    answers = _parse_clinical(form)
...
async def ingest_film(request: Request) -> JSONResponse:
    ...
    if not _ingest_key_ok(request, form):
        return JSONResponse({'ok': False, 'error': 'invalid or missing API key'}, status_code=401)
    sheet = form.get('sheet')
    filename = getattr(sheet, 'filename', None)
    if sheet is None or not filename or (not _has_suffix(filename, FILM_SUFFIXES)):
        return JSONResponse({'ok': False, 'error': "missing film image (field 'sheet')"}, status_code=400)
    answers = _parse_clinical(form)
    cols = _parse_int(form.get('cols'), default=8)
    depth = _parse_int(form.get('depth'), default=64)
    token = uuid.uuid4().hex
    ...
    _latest_digitized.update(label_pretty=_pretty(prediction.label), wmd_pct=wmd_pct, ts=time.time())
    return JSONResponse({'ok': True, 'label': prediction.label, 'label_pretty': _pretty(prediction.label), 'confidence': round(prediction.confidence * 100, 1), 'wmd_probability': wmd_pct, 'probabilities': {name: round(p * 100, 1) for name, p in prediction.probabilities.items()}})
