...
def train_real(
    ...
    if test_manifest is not None:
        ...
        tr_idx, te_idx = train_test_split(
            indices, test_size=config.val_fraction,
            random_state=config.seed, stratify=strat,
        )
    ...
    print(f"Training: {len(train_ds)} scans, "
          f"labels: {dict(zip(*np.unique(train_label_list, return_counts=True)))}")
    ...
    perf = {
        "note": (
            ...
            "(110 real brain MRI scans with radiologist-labeled white matter "
            "hyperintensities from 3 hospitals). The model was trained on "
            "the challenge training set (60 scans) and evaluated on this "
            "completely separate test set. Binary labels: WMH volume <= 5 mL "
            "= 'low burden' (0), > 5 mL = 'significant burden' (1). "
            "Data: Kuijf et al., NeuroImage 2019, doi:10.34894/AECRSD."
        ),
        "detection": {
            "task": "Detection (WMH burden: low vs. significant, threshold 5 mL)",
            "class_names": list(CLASS_NAMES),
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Train NeuroPredict detection model on real MRI data.",
    )
    parser.add_argument(
        "--train-manifest", type=Path, required=True,
        help="Training manifest CSV (from prepare_wmh_data.py).",
    )
    parser.add_argument(
        "--test-manifest", type=Path, default=None,
        help="Held-out test manifest CSV. If omitted, uses a random split of training.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--model-out", type=Path, default=DEFAULT_REAL_MODEL_PATH,
        help=f"Output model path (default: {DEFAULT_REAL_MODEL_PATH}).",
    )
    parser.add_argument(
