...
ETIOLOGY_CLASS_NAMES = ('no_wmd', 'vascular', 'autoimmune', 'genetic', 'metabolic', 'infectious')
ETIOLOGY_LABELS = {'no_wmd': 'No white matter disease', 'vascular': 'Vascular (small-vessel disease)', 'autoimmune': 'Autoimmune (e.g. multiple sclerosis)', 'genetic': 'Genetic (e.g. CADASIL / CARASIL)', 'metabolic': 'Metabolic (e.g. leukodystrophy, B12 deficiency)', 'infectious': 'Infectious (e.g. HIV, Lyme, PML)'}
ETIOLOGY_NEXT_STEPS = {'no_wmd': ['No disease flagged — not a diagnosis; see a doctor if you have symptoms.', 'Protect brain health: exercise, good diet, control BP/sugar/cholesterol.'], 'vascular': ['Share with a doctor or neurologist.', 'Ask about controlling blood pressure, diabetes and cholesterol.', 'Exercise, heart-healthy diet, stop smoking.'], 'autoimmune': ['Ask for a neurology referral to evaluate for MS.', 'May need a contrast MRI of brain/spine and a lumbar puncture.'], 'genetic': ['Consider genetic counseling (CADASIL/NOTCH3, CARASIL/HTRA1, COL4A1).', 'Family members may be screened; manage stroke risk factors.'], 'metabolic': ['See a physician for a metabolic workup (e.g. vitamin B12, thyroid).', 'Some metabolic causes are treatable — early evaluation matters.'], 'infectious': ['See a doctor promptly for an infection workup (HIV, Lyme, etc.).', 'Mention recent infections, travel, tick exposure or fevers.']}
...
CLINICAL_FIELDS = (ClinicalField('age', 'Age (years)', 'age', 'Demographics'), ClinicalField('hypertension', 'High blood pressure', 'binary', 'History'), ClinicalField('diabetes', 'Diabetes', 'binary', 'History'), ClinicalField('prior_stroke', 'Prior stroke or TIA', 'binary', 'History'), ClinicalField('smoking', 'Current or former smoker', 'binary', 'History'), ClinicalField('high_cholesterol', 'High cholesterol', 'binary', 'History'), ClinicalField('autoimmune_history', 'Autoimmune disease (e.g. MS, lupus)', 'binary', 'History'), ClinicalField('recent_cns_infection', 'Recent/chronic CNS infection (HIV, Lyme)', 'binary', 'History'), ClinicalField('metabolic_disorder', 'Metabolic disorder (B12 deficiency, leukodystrophy)', 'binary', 'History'), ClinicalField('memory_problems', 'Memory problems', 'binary', 'Symptoms'), ClinicalField('slow_gait', 'Slow walking / gait changes', 'binary', 'Symptoms'), ClinicalField('balance_problems', 'Balance problems / falls', 'binary', 'Symptoms'), ClinicalField('poor_concentration', 'Reduced concentration', 'binary', 'Symptoms'), ClinicalField('low_mood', 'Low mood / depression', 'binary', 'Symptoms'), ClinicalField('urinary_incontinence', 'Urinary incontinence', 'binary', 'Symptoms'), ClinicalField('apoe4_carrier', 'APOE ε4 carrier', 'binary', 'Genomic'), ClinicalField('notch3_variant', 'NOTCH3 variant (CADASIL)', 'binary', 'Genomic'), ClinicalField('htra1_variant', 'HTRA1 variant (CARASIL)', 'binary', 'Genomic'), ClinicalField('col4a1_variant', 'COL4A1 / COL4A2 variant', 'binary', 'Genomic'), ClinicalField('mthfr_677tt', 'MTHFR C677T (TT genotype)', 'binary', 'Genomic'), ClinicalField('family_history_stroke', 'Family history of stroke / vascular dementia', 'binary', 'Genomic'), ClinicalField('high_wmh_prs', 'Elevated WMH polygenic risk score', 'binary', 'Genomic'))
...
def _ellipsoid(shape: tuple[int, int, int], rng: np.random.Generator) -> np.ndarray:
    ...
    val = ((zz - d / 2) / (d * 0.42 * j[0])) ** 2 + ((yy - h / 2) / (h * 0.4 * j[1])) ** 2 + ((xx - w / 2) / (w * 0.38 * j[2])) ** 2
...
def _add_lesions(vol: np.ndarray, etiology: str, rng: np.random.Generator) -> None:
    ...
    if etiology == 'vascular':
        for _ in range(int(rng.integers(3, 7))):
            _add_blob(vol, (band(0.35, 0.65, 0), band(0.35, 0.65, 1), band(0.3, 0.7, 2)), float(rng.uniform(1.5, 2.6)), float(rng.uniform(0.45, 0.65)))
    elif etiology == 'autoimmune':
        for _ in range(int(rng.integers(2, 5))):
            _add_blob(vol, (band(0.4, 0.6, 0), band(0.42, 0.58, 1), band(0.45, 0.55, 2)), float(rng.uniform(2.0, 3.2)), float(rng.uniform(0.5, 0.7)))
    elif etiology == 'genetic':
        for _ in range(int(rng.integers(1, 3))):
            z, y = (band(0.4, 0.6, 0), band(0.3, 0.45, 1))
            for x in (band(0.25, 0.38, 2), w - band(0.25, 0.38, 2)):
                _add_blob(vol, (z, y, x), float(rng.uniform(1.8, 2.8)), float(rng.uniform(0.5, 0.68)))
    elif etiology == 'metabolic':
        for _ in range(int(rng.integers(6, 11))):
            _add_blob(vol, (band(0.3, 0.7, 0), band(0.3, 0.7, 1), band(0.3, 0.7, 2)), float(rng.uniform(1.2, 2.0)), float(rng.uniform(0.35, 0.5)))
    elif etiology == 'infectious':
        for _ in range(int(rng.integers(1, 4))):
            _add_blob(vol, (band(0.3, 0.7, 0), band(0.3, 0.7, 1), band(0.25, 0.75, 2)), float(rng.uniform(2.6, 4.0)), float(rng.uniform(0.5, 0.72)))
...
def make_dataset(cfg: TrainConfig, n_per_class: int, seed: int):
    ...
    return (torch.tensor(np.stack(vols)), torch.tensor(np.stack(clins)), torch.tensor(bins), torch.tensor(etis))
IMAGE_EMBED_DIM = 64
class ConvBlock(nn.Module):
    def __init__(self, cin: int, cout: int) -> None:
        super().__init__()
        self.block = nn.Sequential(nn.Conv3d(cin, cout, 3, padding=1, bias=False), nn.GroupNorm(min(8, cout), cout), nn.ReLU(inplace=True), nn.MaxPool3d(2))
    def forward(self, x):
        return self.block(x)
...
def grad_cam(model: MultimodalWMDClassifier, volume: torch.Tensor, clinical: torch.Tensor, target: int) -> np.ndarray:
    model.eval()
...
def _report(labels, preds, probs, class_names) -> dict:
    ...
    if len(set(labels)) > 1:
        if n == 2:
            ...
            metrics['roc_auc_macro'] = float(roc_auc_score(labels, probs, multi_class='ovr', average='macro', labels=list(range(n))))
    return {'class_names': list(class_names), 'confusion_matrix': cm.tolist(), 'n_samples': len(labels), 'metrics': metrics}
...
def train_everything(cfg: TrainConfig | None=None, save: bool=True) -> dict:
    ...
    det_labels, det_preds, det_probs = _predict_all(det, DataLoader(TensorDataset(tv, tb), batch_size=cfg.batch_size), device, False)
    mm_labels, mm_preds, mm_probs = _predict_all(mm, DataLoader(TensorDataset(tv, tc, te), batch_size=cfg.batch_size), device, True)
    performance = {'note': 'Metrics on a fresh synthetic test set the models never trained on. Synthetic data demonstrates the pipeline, not clinical performance.', 'detection': {'task': 'Detection (white matter disease: yes / no)', **_report(det_labels, det_preds, det_probs, CLASS_NAMES)}, 'etiology': {'task': 'Cause / etiology (5 causes + healthy)', **_report(mm_labels, mm_preds, mm_probs, ETIOLOGY_CLASS_NAMES)}}
    artifacts = {'det_state': det.state_dict(), 'mm_state': mm.state_dict(), 'performance': performance, 'shape': cfg.shape}
    if save:
...
class Predictor:
    ...
    def predict(self, volume: np.ndarray, answers: dict[str, float]) -> dict:
        ...
        return {'wmd_probability': float(det_prob[1]), 'detected': bool(det_prob[1] >= 0.5), 'etiology': eti_name, 'etiology_label': ETIOLOGY_LABELS[eti_name], 'etiology_confidence': float(eti_prob[eti_idx]), 'cause_probs': sorted([(ETIOLOGY_LABELS[n], float(p)) for n, p in zip(ETIOLOGY_CLASS_NAMES, eti_prob) if n != 'no_wmd'], key=lambda t: t[1], reverse=True), 'next_steps': ETIOLOGY_NEXT_STEPS[eti_name], 'salient_slice': salient}
...
def _cell_style(count: int, peak: int, diagonal: bool) -> str:
    ...
    if diagonal:
        return f'background: rgba(34,197,94,{0.1 + 0.7 * intensity:.3f})'
...
def _confusion_html(rep: dict) -> str:
    ...
    for i, (name, row) in enumerate(zip(names, matrix)):
        cells = ''.join((f'<td style="{_cell_style(c, peak, i == j)}" title="{name} predicted as {names[j]}: {c}">{c}</td>' for j, c in enumerate(row)))
        rows += f"<tr><th class='rowlab'>{name}</th>{cells}</tr>"
    metric_map = {'accuracy': 'Accuracy', 'roc_auc': 'ROC-AUC', 'roc_auc_macro': 'ROC-AUC (macro)', 'sensitivity': 'Sensitivity', 'specificity': 'Specificity'}
    cards = ''.join((f'<div class="card2"><span class="v">{v:.3f}</span><span class="l">{metric_map.get(k, k)}</span></div>' for k, v in rep['metrics'].items()))
    cards += f'<div class="card2 muted"><span class="v">{rep['n_samples']}</span><span class="l">Test scans</span></div>'
    return f"<h3>{rep['task']}</h3><div class='cards'>{cards}</div><div class='cmwrap'><table class='cm'><thead><tr><th class='rowlab'>Actual \\ Predicted</th>{head}</tr></thead><tbody>{rows}</tbody></table></div>"
...
def build_app():
    ...
    def index():
        if predictor is None:
            return _page('Predict', "<div class='card'><div class='banner banner-warn'>No model loaded. Run <code>python neuropredict_all_in_one.py train</code>.</div></div>")
        opts = ''.join((f'<option value={i}>{ETIOLOGY_LABELS[n]}</option>' for i, n in enumerate(ETIOLOGY_CLASS_NAMES)))
    ...
    async def predict(request: Request):
        ...
        causes = ''.join((f"<tr><td>{lbl}</td><td><span class=bar style='width:{int(p * 200)}px'></span> {p * 100:.1f}%</td></tr>" for lbl, p in r['cause_probs']))
    ...
    def performance():
        if predictor is None:
            return _page('Performance', "<div class='card'><div class='banner banner-warn'>No model loaded. Run the training command first.</div></div>")
        perf = predictor.performance
        body = f"<section class=card><h2>Model performance</h2><p class=muted>Measured on a held-out test set the models never trained on. Each confusion matrix compares predictions (columns) to the truth (rows): the diagonal is correct, off-diagonal is a mistake.</p><div class='banner banner-info'>{perf['note']}</div>{_confusion_html(perf['detection'])}{_confusion_html(perf['etiology'])}</section>"
