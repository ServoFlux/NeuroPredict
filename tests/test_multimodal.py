...
from wmd.clinical import CLINICAL_FIELD_NAMES, NUM_CLINICAL_FEATURES, encode_clinical, make_clinical
from wmd.config import CLASS_NAMES, ETIOLOGY_CLASS_NAMES, PreprocessConfig, TrainConfig
from wmd.dataset import MultimodalManifestDataset
from wmd.inference import MultimodalWMDPredictor
from wmd.model import build_multimodal_model
from wmd.synthetic import generate_dataset, make_etiology_volume, make_volume
from wmd.train import train_multimodal
...
def test_multimodal_dataset(tmp_path: Path) -> None:
    manifest = generate_dataset(tmp_path, n_per_class=3, shape=(32, 32, 32), with_clinical=True)
    ds = MultimodalManifestDataset(manifest, preprocess=PreprocessConfig(target_shape=(32, 32, 32)))
    assert len(ds) == 6
    vol, clin, label = ds[0]
...
def test_train_predict_and_attribution(tmp_path: Path) -> None:
    manifest = generate_dataset(tmp_path / 'data', n_per_class=8, shape=(32, 32, 32), with_clinical=True)
    model_path = tmp_path / 'mm.pt'
    config = TrainConfig(epochs=3, batch_size=4, preprocess=PreprocessConfig(target_shape=(32, 32, 32)))
    ...
    assert abs(attr.image_share + attr.clinical_share - 1.0) < 1e-06
def test_make_clinical_etiology_profiles() -> None:
    ...
    assert np.mean([a['notch3_variant'] for a in genetic]) > np.mean([a['notch3_variant'] for a in vascular])
    assert np.mean([a['hypertension'] for a in vascular]) > np.mean([a['hypertension'] for a in genetic])
def test_multiclass_dataset_and_prediction(tmp_path: Path) -> None:
    manifest = generate_dataset(tmp_path / 'data', n_per_class=6, shape=(32, 32, 32), with_clinical=True, multiclass=True)
    ds = MultimodalManifestDataset(manifest, preprocess=PreprocessConfig(target_shape=(32, 32, 32)))
    ...
    model_path = tmp_path / 'mm.pt'
    config = TrainConfig(epochs=2, batch_size=4, preprocess=PreprocessConfig(target_shape=(32, 32, 32)))
    train_multimodal(manifest, config=config, model_path=model_path)
    predictor = MultimodalWMDPredictor(model_path)
    assert predictor.class_names == list(ETIOLOGY_CLASS_NAMES)
    volume = make_etiology_volume(ETIOLOGY_CLASS_NAMES.index('vascular'), shape=(40, 40, 40), rng=np.random.default_rng(0))
