"""Tests for the research severity indicator."""
...
from wmd.config import assess_severity  # noqa: E402


def test_severity_buckets() -> None:
    assert assess_severity(0.90).level == "Severe"
    assert assess_severity(0.85).level == "Severe"  # boundary inclusive
    assert assess_severity(0.75).level == "Moderate"
    assert assess_severity(0.70).level == "Moderate"  # boundary inclusive
