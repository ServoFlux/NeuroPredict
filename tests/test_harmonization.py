...
from wmd.harmonization import bias_field_correct, otsu_brain_mask, white_stripe_normalize, zscore_normalize
...
def test_zscore_normalize_stats() -> None:
    ...
    brain = out[mask]
    assert abs(float(brain.mean())) < 0.0001
    assert abs(float(brain.std()) - 1.0) < 0.0001
...
def test_preprocess_harmonization_modes_shape_and_default_unchanged() -> None:
    ...
    for mode in ('zscore', 'whitestripe'):
        out = preprocess_volume(vol, PreprocessConfig(target_shape=target, intensity_norm=mode, bias_correct=True))
