"""Tests for cross-scanner harmonization preprocessing."""
...
from wmd.config import PreprocessConfig  # noqa: E402
from wmd.harmonization import (  # noqa: E402
    bias_field_correct,
    otsu_brain_mask,
...
from wmd.synthetic import make_volume  # noqa: E402


def _shaded_volume(shape=(32, 32, 32)) -> np.ndarray:
    """A synthetic brain with a smooth left-to-right brightness gradient."""
...
def test_otsu_mask_separates_brain_from_background() -> None:
    ...
    # The brain occupies a meaningful but not total fraction of the volume.
    frac = float(mask.mean())
    assert 0.05 < frac < 0.95
    # Foreground should be brighter than background on average.
...
def test_bias_correction_reduces_shading() -> None:
    ...
    # Compare the mean intensity of the left vs right half of the brain: the
    # gradient makes them differ; correction should shrink that gap.
...
def test_whitestripe_is_scanner_scale_invariant() -> None:
    """Doubling raw intensities (a different scanner's units) should yield a
    nearly identical WhiteStripe-normalized volume."""
...
def test_preprocess_harmonization_modes_shape_and_default_unchanged() -> None:
    ...
    for mode in ("zscore", "whitestripe"):
        ...
        # A harmonizing mode changes the output vs the default minmax pipeline.
