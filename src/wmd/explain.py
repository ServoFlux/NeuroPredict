...
def grad_cam(model: nn.Module, input_tensor: torch.Tensor, class_idx: int, target_layer: nn.Module | None=None) -> np.ndarray:
    ...
    def backward_hook(_module: nn.Module, _grad_in: object, grad_out: tuple[torch.Tensor, ...]) -> None:
    ...
    try:
        ...
        handle_fwd.remove()
        handle_bwd.remove()
    cam = F.interpolate(cam[None, None].detach(), size=tuple(input_tensor.shape[2:]), mode='trilinear', align_corners=False)[0, 0].numpy()
...
def overlay_cam_on_slice(base_slice: np.ndarray, cam_slice: np.ndarray, alpha: float=0.55) -> np.ndarray:
