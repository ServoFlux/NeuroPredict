...
def _save_cam_slice(processed: np.ndarray, cam: np.ndarray, input_png: str | Path, overlay_png: str | Path) -> int:
...
class WMDPredictor:
    def __init__(self, model_path: str | Path=DEFAULT_MODEL_PATH) -> None:
        ...
        if not self.model_path.exists():
            raise FileNotFoundError(f'Model checkpoint not found at {self.model_path}. Train one first (see scripts/train_demo.py).')
        ...
        self.preprocess = PreprocessConfig(target_shape=tuple(checkpoint['target_shape']), clip_percentiles=tuple(checkpoint['clip_percentiles']))
    ...
    def predict_volume(self, volume: np.ndarray) -> Prediction:
        ...
        return Prediction(label=self.class_names[idx], label_index=idx, confidence=float(probs[idx]), probabilities={name: float(p) for name, p in zip(self.class_names, probs)})
...
class MultimodalWMDPredictor:
    def __init__(self, model_path: str | Path=DEFAULT_MULTIMODAL_MODEL_PATH) -> None:
        self.model_path = Path(model_path)
        if not self.model_path.exists():
            raise FileNotFoundError(f'Multimodal model checkpoint not found at {self.model_path}. Train one first (see scripts/train_demo.py).')
        checkpoint = torch.load(str(self.model_path), map_location='cpu', weights_only=False)
        self.class_names: list[str] = checkpoint['class_names']
        self.clinical_fields: list[str] = checkpoint['clinical_fields']
        self.preprocess = PreprocessConfig(target_shape=tuple(checkpoint['target_shape']), clip_percentiles=tuple(checkpoint['clip_percentiles']))
        self.val_metrics: dict[str, float] = checkpoint.get('val_metrics', {})
        self.model = build_multimodal_model(num_clinical_features=checkpoint['num_clinical_features'], num_classes=checkpoint['num_classes'])
        self.model.load_state_dict(checkpoint['state_dict'])
        self.model.eval()
        self._healthy_index = self.class_names.index('no_wmd') if 'no_wmd' in self.class_names else None
    ...
    @torch.no_grad()
    def predict(self, volume: np.ndarray, answers: dict[str, float]) -> tuple[Prediction, ModalityAttribution]:
        tensor = preprocess_volume(volume, self.preprocess)[None]
        ...
        idx = int(probs.argmax())
        prediction = Prediction(label=self.class_names[idx], label_index=idx, confidence=float(probs[idx]), probabilities={n: float(p) for n, p in zip(self.class_names, probs)})
        ...
        image_share = abs(image_delta) / total if total > 1e-06 else 0.5
        attribution = ModalityAttribution(combined=combined, baseline=baseline, image_delta=image_delta, clinical_delta=clinical_delta, image_share=image_share, clinical_share=1.0 - image_share)
        return (prediction, attribution)
    def predict_path(self, path: str | Path, answers: dict[str, float]) -> tuple[Prediction, ModalityAttribution]:
