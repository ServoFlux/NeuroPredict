...
def _to_uint8(arr: np.ndarray, lo: float, hi: float) -> np.ndarray:
    norm = np.clip((arr - lo) / (hi - lo + 1e-08), 0.0, 1.0)
    return (norm * 255).astype(np.uint8)
def grid_shape_for_depth(depth: int, cols: int) -> tuple[int, int]:
    cols = max(1, int(cols))
    rows = max(1, math.ceil(depth / cols))
    return (rows, cols)
def contact_sheet_from_volume(volume: np.ndarray, cols: int=8, cell: int=64) -> np.ndarray:
    ...
    for idx in range(depth):
        r, c = divmod(idx, cols)
        tile = Image.fromarray(_to_uint8(volume[idx], lo, hi)).resize((cell, cell), Image.BILINEAR)
        sheet[r * cell:(r + 1) * cell, c * cell:(c + 1) * cell] = np.asarray(tile)
    return sheet
def save_contact_sheet(volume: np.ndarray, out_path: str | Path, cols: int=8, cell: int=64) -> Path:
    from PIL import Image
...
def volume_from_contact_sheet(image: str | Path | np.ndarray, rows: int, cols: int, depth: int | None=None, auto_crop: bool=True) -> np.ndarray:
    rows = max(1, int(rows))
    cols = max(1, int(cols))
