...
def otsu_brain_mask(volume: np.ndarray) -> np.ndarray:
    ...
    mean_fg = np.divide(global_mean - cumsum_mean, weight_fg, out=np.zeros_like(cumsum_mean), where=weight_fg > 0)
...
def bias_field_correct(volume: np.ndarray, sigma: float=8.0, mask: np.ndarray | None=None) -> np.ndarray:
...
def white_stripe_normalize(volume: np.ndarray, mask: np.ndarray | None=None, width: float=0.05) -> np.ndarray:
    volume = volume.astype(np.float32)
    if mask is None:
        mask = otsu_brain_mask(volume)
    brain = volume[mask]
