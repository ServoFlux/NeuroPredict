...
def _encode_multipart(fields: dict[str, str], image_bytes: bytes, image_name: str):
    ...
    body.write(f'--{boundary}{crlf}'.encode())
    body.write(f'Content-Disposition: form-data; name="sheet"; filename="{image_name}"{crlf}'.encode())
...
def main() -> int:
    ...
    parser.add_argument('--etiology', default='vascular', choices=list(ETIOLOGY_CLASS_NAMES), help='Synthetic volume to generate when --source is not given')
